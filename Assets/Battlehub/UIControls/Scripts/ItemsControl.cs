...
namespace Battlehub.UIControls
{
    ...
    public class ItemsControl<TDataBindingArgs> : MonoBehaviour, IPointerDownHandler, IDropHandler where TDataBindingArgs : ItemDataBindingArgs, new()
    {
        ...
        public void ExternalItemDrag(PointerEventData eventData)
        {
            if (m_dropTarget != null)
            {
				...
				if (m_itemContainers != null) {
					for (int i = 0; i < m_itemContainers.Count; ++i) {
						...
						if (container != null) {
							...
							container.LayoutElement.preferredWidth = 80;
							container.LayoutElement.preferredHeight = 80;// WARNING
						}
					}
				} 
            }
        ...
        protected ItemContainer InstantiateItemContainer(int siblingIndex)
        {
			...
			container.GetComponent<LayoutElement>().preferredWidth = 80;
			container.GetComponent<LayoutElement>().preferredHeight = 80;
